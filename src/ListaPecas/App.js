...
function ListaPecas() {
  ...
  const peticionGet=async()=>{
    await axios.get(baseUrl+'/lista-pecas')
    .then(response=>{
  ...
  const peticionPut=async()=>{
    await axios.patch(baseUrl+"/lista-pecas/"+gestorSeleccionado.id, gestorSeleccionado)
    .then(response=>{
      var respuesta=response.data;
      var dataAuxiliar=data;
      dataAuxiliar.map( (gestor) => {
        if(gestor.id===gestorSeleccionado.id){
          gestor.fk_idPeca=respuesta.fk_idPeca;
          gestor.fk_idProduto=respuesta.fk_idProduto;
          gestor.produtos=respuesta.produtos;
          gestor.pecas=respuesta.pecas;
        }
      });
  ...
  const peticionDelete=async()=>{
    await axios.delete(baseUrl+"/lista-pecas/"+gestorSeleccionado.id)
    .then(response=>{
            ...
            <tbody>
              {data.map(gestor => (
                <tr key={gestor.id}>
                  ...
                  <td align='right'>{gestor.produtos.nomeproduto}</td>
                  <td align='right'>{gestor.pecas.nomepeca}</td>
                  <td align='center'>
